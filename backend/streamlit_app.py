...
# Function to check AI model settings
def test_ai_settings():
    ...
    return f"AI settings look good! Model: {model or 'default'}"
...
with st.sidebar:
    ...
    if st.button("Test AI Settings"):
        st.info(test_ai_settings())
                ...
                if isinstance(logs, dict) and "error" in logs:
                    ...
                    for log in logs:
                        with st.expander(f"{log.get('agent_name', 'Unknown')} - {log.get('thinking_stage', 'Unknown')} - {log.get('created_date', 'Unknown date')}"):
                            ...
                            with col2:
                                st.write(f"**Agent ID:** {log.get('agent_ref_id', 'N/A')}")
