...
from config.settings import get_model_deployment_name
...
class ChatbotManager:
    ...
    def __init__(self, connection_string):
        ...
        # Shared web search plugin, attached to the political/tariff/logistics risk agents.
        try:
            self.web_search_plugin = WebSearchPlugin()
        except Exception as e:
            print(f"Error initializing web search plugin: {e}")
    ...
    async def initialize_session(self, session_id):
        ...
        try:
            # Model settings
            ai_agent_settings = SimpleNamespace(
    ...
    async def _create_agents_for_session(self, client, ai_agent_settings, session_id, conversation_id):
        ...
        web_search = [self.web_search_plugin] if self.web_search_plugin else []
        ...
        # Create political risk agent with web search and political risk JSON plugin
        ...
        agents[POLITICAL_RISK_AGENT] = await create_or_reuse_agent(
            ...
            plugins=[political_logging, political_risk_json_plugin, citation_logger_plugin, *web_search]
        )

        # Create tariff risk agent with web search
        ...
        agents[TARIFF_RISK_AGENT] = await create_or_reuse_agent(
            ...
            plugins=[tariff_logging, *web_search]
        )

        # Create logistics risk agent with web search
        ...
        agents[LOGISTICS_RISK_AGENT] = await create_or_reuse_agent(
            ...
            plugins=[logistics_logging, *web_search]
        )
            ...
            if hasattr(response_message, 'url_citation_annotations') and response_message.url_citation_annotations:
                for annotation in response_message.url_citation_annotations:
                    citation = {
                        ...
                        "source": "Web Search"  # Default source name
                    }
