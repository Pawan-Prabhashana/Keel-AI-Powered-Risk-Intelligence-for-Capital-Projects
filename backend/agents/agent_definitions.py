"""Complete agent definitions with enhanced web search guidance and support for both thinking_stage_output and agent_output."""
...
- agent_ref_id: Get by calling log_agent_get_agent_id()
...
def get_political_risk_agent_instructions(agent_id=None):
    """Returns political risk agent instructions with enhanced thinking stages for better web search citation handling."""
    return f"""
...
3. Use web search to find relevant news about political risks affecting supply chains
4. Report those risks in a clear, structured format with proper tables
5. Ensure all your sources are properly cited using the built-in web search citations

IMPORTANT: Document your thinking process at each step by calling log_agent_thinking with:
- agent_name: "POLITICAL_RISK_AGENT"
- thinking_stage: One of "analysis_start", "json_extraction", "web_search_attempt", "web_search_results", "risk_identification", "risk_assessment", "recommendations"
- thought_content: Detailed description of your thoughts at this stage
- conversation_id: Use the same ID throughout a single analysis run
- session_id: the chat session id
- agent_ref_id: {agent_id if agent_id else 'Get by calling log_agent_get_agent_id()'}
- model_deployment_name: The model_deployment_name of the agent
- thread_id: Get by calling log_agent_get_thread_id()
- thinking_stage_output: Include specific outputs for this thinking stage that you want preserved separately
...
4. CRITICAL: FOR WEB SEARCH - Follow these steps carefully:
   a. Extract the search query from the scheduler's JSON under "searchQuery.political"
   b. Call log_agent_thinking with thinking_stage="web_search_attempt" and include:
      * The exact query you are about to use
      * Your search strategy
   c. IMPORTANT: To perform web search, you MUST use the built-in search capability that is already available to you
   ...
   d. Call log_agent_thinking with thinking_stage="web_search_results" and include:
...
1. Brief overview of how you used web search
...
def get_tariff_risk_agent_instructions(agent_id=None):
    """Returns tariff risk agent instructions with enhanced web search guidance."""
    return f"""
...
4. Use web search to find relevant news published within the last 30 days
5. Report those risks in a clear, structured format with proper tables

IMPORTANT: Document your thinking process at each step by calling log_agent_thinking with:
- agent_name: "TARIFF_RISK_AGENT"
- thinking_stage: One of "analysis_start", "location_extraction", "web_search_attempt", "web_search_results", "tariff_research", "risk_assessment", "recommendations"
- thought_content: Detailed description of your thoughts at this stage
- conversation_id: Use the same ID throughout a single analysis run
- session_id: the chat session id
- agent_ref_id: {agent_id if agent_id else 'Get by calling log_agent_get_agent_id()'}
...
4. CRITICAL: FOR WEB SEARCH - Follow these simplified steps:
   a. Call log_agent_thinking with thinking_stage="web_search_attempt"
   ...
   d. Call log_agent_thinking with thinking_stage="web_search_results" and include the search results
...
def get_logistics_risk_agent_instructions(agent_id=None):
    """Returns logistics risk agent instructions with enhanced web search guidance."""
    return f"""
...
- thinking_stage: One of "analysis_start", "port_extraction", "web_search_attempt", "web_search_results", "logistics_research", "risk_assessment", "recommendations"
- thought_content: Detailed description of your thoughts at this stage
- conversation_id: Use the same ID throughout a single analysis run
- session_id: the chat session id
- agent_ref_id: {agent_id if agent_id else 'Get by calling log_agent_get_agent_id()'}
