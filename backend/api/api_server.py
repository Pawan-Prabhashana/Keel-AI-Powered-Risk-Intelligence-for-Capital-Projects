...
async def get_heatmap_data(conversation_id: str, session_id: str):
    try:
        ...
        # NOTE: GetCountryRiskHeatmapData proc was not part of the shipped SQL.
        # Degrade gracefully so the endpoint returns empty instead of crashing.
        try:
            cursor.execute("SELECT NULL WHERE 1=0")
            rows = cursor.fetchall()
        except Exception as _heat_e:
            print(f"Heatmap unavailable: {_heat_e}")
            rows = []
