...
class LoggingPlugin:
    ...
    def log_agent_get_thread_id(self) -> str:
        """Returns a stable thread id for this conversation."""
        try:
            if not self._current_thread_id:
                import uuid
                self._current_thread_id = "thread_" + uuid.uuid4().hex[:16]
    ...
    def log_agent_thinking(self, agent_name: str, thinking_stage: str, thought_content: str, 
                        ...
                        agent_ref_id: str = None, model_deployment_name: str = None,
        ...
        Args:
            ...
            agent_ref_id: Agent identifier
            model_deployment_name: Name of the model deployment
            thread_id: Conversation thread id (if available)
        ...
        try:
            ...
            # If agent_ref_id is None or "Get by calling log_agent_get_agent_id()", try to get it
            if agent_ref_id is None or agent_ref_id == "Get by calling log_agent_get_agent_id()":
                try:
                    agent_ref_id = self.log_agent_get_agent_id()
                except Exception as e:
                    print(f"Error getting agent ID: {e}")
                    agent_ref_id = "agent_id_retrieval_error"
            ...
            try:
                ...
                cursor.execute("""
                    ...
                    conversation_id, session_id, agent_ref_id, model_deployment_name, thread_id,
    ...
    def log_agent_event(self, agent_name: str, action: str, result_summary: str = None, 
        ...
        try:
            ...
            cursor.execute("INSERT INTO dim_agent_event_log (event_id, agent_name, event_time, action, result_summary, conversation_id, session_id, user_query, agent_output) VALUES (NEWID(), ?, GETDATE(), ?, ?, ?, ?, ?, ?)", params)
        ...
        try:
            ...
            query = f"""
                SELECT TOP {limit} 
                    ...
                    conversation_id, session_id, agent_ref_id, model_deployment_name, 
