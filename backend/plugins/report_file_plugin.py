...
try:
    ...
    Document = object  # fallback so type annotations still resolve without Spire

# Reports are saved to the local reports directory.
STORAGE_UPLOAD_ENABLED = False

class ReportFilePlugin:
    """A plugin for creating Word reports and saving them locally."""
    ...
    def __init__(self, connection_string, storage_connection_string=None):
        ...
        # Reports are stored locally; no external storage client needed.
        self.blob_service_client = None
    
    @kernel_function(description="Saves a report to a Word document")
    def save_report_to_file(self, report_content: str, session_id: str, 
                          conversation_id: str, report_title: str = None) -> str:
        """Saves a report to Word document and uploads to cloud storage with improved error handling.
        ...
        try:
            ...
            # Upload to cloud storage with detailed error handling
            blob_url = None
            try:
                if self.blob_service_client and STORAGE_UPLOAD_ENABLED:
                    print(f"Uploading to cloud storage...")
                    blob_url = self._upload_to_data_lake(docx_filepath, docx_filename)
                    print(f"Successfully uploaded to cloud storage: {blob_url}")
                else:
                ...
                print(f"Error uploading to cloud storage: {upload_error}")
                traceback.print_exc()
    ...
    def _upload_to_data_lake(self, filepath: str, filename: str) -> str:
        """Uploads a file to optional cloud storage with improved error handling.
        ...
        try:
            ...
            if not self.blob_service_client or not STORAGE_UPLOAD_ENABLED:
                print("Blob service client not available, cannot upload to cloud storage")
    ...
    def _log_report_to_database(self, session_id: str, conversation_id: str, 
        ...
        try:
            ...
            try:
                cursor.execute(
                        "INSERT INTO fact_risk_report (session_id, conversation_id, filename, blob_url, report_type) VALUES (?, ?, ?, ?, ?)",
                        (session_id, conversation_id, filename, blob_url, "comprehensive")
                    )
